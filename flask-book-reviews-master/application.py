...
app = Flask(__name__)
bcrypt = Bcrypt(app)
item = mysql.connector.connect(user='root', password='123456', host='127.0.0.1', port='3306', database='bookdb')
db = item.cursor()
...
def register():
    ...
    if request.method == "POST":
        try: 
            ...
            result = db.execute('INSERT INTO accounts VALUES (NULL,%s, %s,%s)', (usern, passw_hash, email))
            item.commit()
...
def login():
    ...
    if request.method == "POST":
        ...
        db.execute('SELECT * FROM accounts WHERE username = %s AND password = %s', (usern, passw))
        result =db.fetchone()
